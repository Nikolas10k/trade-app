export type ChecklistValue = Record<string, boolean>;

export type ChecklistItem = { id: string; label: string };
export type ChecklistPhase = { phase: string; title: string; items: ChecklistItem[] };

export const CHECKLIST_PHASES: ChecklistPhase[] = [
  {
    phase: "f1",
    title: "Fase 1 — Contexto e plano",
    items: [
      { id: "f1_tendencia", label: "Tendência identificada" },
      { id: "f1_quatro_velas", label: "Análise das últimas 4 velas" },
      { id: "f1_pips_ref", label: "Pips de referência definidos" },
      { id: "f1_canal_mapeado", label: "Canal mapeado" },
      { id: "f1_canal2_confirmado", label: "Segundo canal confirmado" },
    ],
  },
  {
    phase: "f2",
    title: "Fase 2 — Dimensionamento",
    items: [
      { id: "f2_lote_definido", label: "Tamanho do lote definido" },
      { id: "f2_pips_stop", label: "Pips do stop definidos" },
      { id: "f2_rr_agradavel", label: "R/R agradável" },
      { id: "f2_margem_costura", label: "Margem de costura considerada" },
    ],
  },
  {
    phase: "f3",
    title: "Fase 3 — Confirmação de entrada",
    items: [{ id: "f3_vela_fora_canais", label: "Vela fora dos canais" }],
  },
  {
    phase: "f4",
    title: "Fase 4 — Gestão da posição",
    items: [
      { id: "f4_parcial_feita", label: "Parcial realizada" },
      { id: "f4_stop_0x0", label: "Stop levado a 0x0" },
      { id: "f4_stop_conduzido", label: "Stop conduzido" },
    ],
  },
];

/** Pré-trade = f1 + f2 + f3 (10 itens). Gestão = f4 (3 itens). Total: 13. */
export const PRE_TRADE_KEYS: readonly string[] = CHECKLIST_PHASES.filter((p) => p.phase !== "f4").flatMap(
  (p) => p.items.map((i) => i.id),
);
export const MANAGEMENT_KEYS: readonly string[] = CHECKLIST_PHASES.filter((p) => p.phase === "f4").flatMap(
  (p) => p.items.map((i) => i.id),
);
export const ALL_CHECKLIST_KEYS: readonly string[] = [...PRE_TRADE_KEYS, ...MANAGEMENT_KEYS];
